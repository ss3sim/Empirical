...
plot_ssb_with_mare <- function(data){
  ...
  par(mar=c(0,0,0,0), oma=c(4,4,6,3))
  ...
  for(ii in 1:length(plot.order)){
    ...
    #add mare value
    text(unique(temp$mare), x= 97, y = .90, cex = 1.2)

    text(x = 0, y=.90, labels=paste0(letters[ii],"."))
    if(ii == 1) mtext(side = 3, text = 'Age + Length', line = 2.5, cex = 1.2, adj = 0)
    if(ii == 7) mtext(side = 3, text = 'Weight-at-Age', line = .25, cex = 1.2, adj = 0)
    if(ii %in% c(1, 2, 3)) mtext(side = 3, line = .5, text = d.amount[ii], adj = 0)
    if(ii %in% c(3, 9)) mtext(side = 4, text = "Time-Invariant", cex = 1, line = .5)
    if(ii %in% c(6, 12)) mtext(side = 4, text = "Time-Varying", cex = 1, line = .5)
    if(ii %in% c(1, 4, 7, 10)) axis(side = 2, at = c(-.5, 0, .5))
    # if(ii %in% c(4, 5, 6)) axis(side = 1, labels = FALSE)
    if(ii %in% c(10, 11, 12)) axis(side = 1, cex.axis = 1)
  }
