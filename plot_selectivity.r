...
plot_selex <- function(d, yy = .1){
  ...
  for(ii in 1:6){
    ...
    # cbind(temp, temp1)
    plot(0, 0, type = 'n', xlim = c(.9, 4.5), ylim = c(-yy - .05, yy + .05),
    ...
    with(temp, {
      ...
      text(x = xvalue, y = yy + .02, label = mare, col = 'black', cex = 1.2)
    ...
    with(temp1, {
      ...
      text(x = xvalue, y = yy + .03, label = mare, col = 'gray45', cex = 1.2)
